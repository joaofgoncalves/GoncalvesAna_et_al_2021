...
#setwd("D:/MyDocs/Dropbox/MSc_AGoncalves")
...
selVarsClim <- paste("bio_",c(3,6,12),sep="")

#selVarsForest <- c("broadlf_cover", "conif_cover", "tree_density")
selVarsForest <- c("broadlf_cover", "tree_density")

#selVarsRemoteSensing <- c("evi_IQR_mb","EVI_median_mb")
selVarsRemoteSensing <- c("EVI_median_mb")

# selVarsSoil <- c("awc_lucas_ib", "bulk_dens_lucas_ib", "coarse_frag_lucas_ib",
#                  "sand_lucas_ib", "STU_EU_DEPTH_ROOTS_ib", "STU_EU_T_OC_ib")
# selVarsSoil <- c("STU_EU_T_OC_ib")
# selVarsTopo <- c("twi_ib")
  
## ------------------------------------------------------------------------- ##
# -- clusters
# Cluster 1       -- T. semiaptera                         "cluster 1"
# Cluster 2       -- T. iberica                            "cluster 2"
# Cluster 4       -- most phylogenetically related species "cluster 4"
#
# -- species
# Ariasella sp. 1 -- T. ebejeri                            "Ariasella sp.1"
# Ariasella sp. 2 -- T. stenoptera                         "Ariasella sp.2"
# Ariasella sp. 3 -- T. cantabrica                         "Ariasella sp.3" 
#
#
## ------------------------------------------------------------------------ ##
...
spNameSelected <- "Ariasella lusitanica"
...
climVars <- list.files("D:/R-dev/QuercusSDMs/DATA_/RASTER/Clim/Present/v2.1", pattern=".tif$",
                       full.names=TRUE)
...
current <- stack(c(climVars, forestVars, remoteSensingVars))
...
myBiomodData <- BIOMOD_FormatingData(resp.var = spRecordPoints,
                                     ...
                                     PA.nb.absences = biomod2plus:::powerFunPAnumberCalc(Npresences), # Nr of pseudo-absences
...
myBiomodModelOut <- BIOMOD_Modeling(
  ...
  VarImport = 10, # Nr of rounds to evaluate variables
...
# quantileThresh <- quantile(evalDF.TSS, probs=0.9, na.rm=TRUE)
# if(quantileThresh==1) quantileThresh <- quantile(evalDF.TSS, probs=0.85, na.rm=TRUE)
# if(quantileThresh==1) quantileThresh <- quantile(evalDF.TSS, probs=0.75, na.rm=TRUE)


selMods <- biomod2plus:::twoStepBestModelSelection(myBiomodModelOut, 
                                     evalMetric = "TSS", 
                                     nrBestAlgos = 6, 
